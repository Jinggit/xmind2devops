...
def parse_a_test_step(step_dict):
    ...
    expected_testdata_topic = step_dict.get('topics', [])
    if expected_testdata_topic:  # have testdata
        expected_testdata_topic = expected_testdata_topic[0]
        expected_expectedresults_topic = expected_testdata_topic.get('topics', [])
        if expected_expectedresults_topic: #have expected result
            expected_expectedresults_topic = expected_expectedresults_topic[0]
            test_step.testcasedata = expected_testdata_topic['title']  # one test step action, one test data
            test_step.expectedresults = expected_expectedresults_topic['title']  # one test step action, one test expected result
            markers = expected_expectedresults_topic['markers']
